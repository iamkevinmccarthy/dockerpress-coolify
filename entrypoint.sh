...
function update_wp_config() {
  ...
  wp config set WP_SITEURL "https://$SERVICE_FQDN_WORDPRESS_OLS" --add --type=constant --path=/var/www/html
  wp config set WP_HOME "https://$SERVICE_FQDN_WORDPRESS_OLS" --add --type=constant --path=/var/www/html
...
function install_wordpress() {
  ...
  if [ ! -e /var/www/html/wp-config.php ]; then
    ...
    if ! $(wp core is-installed --path=/var/www/html); then
      echo "Installing Wordpress for $SERVICE_FQDN_WORDPRESS_OLS ..."
      wp core install --url=$SERVICE_FQDN_WORDPRESS_OLS \
        --title=Wordpress \
