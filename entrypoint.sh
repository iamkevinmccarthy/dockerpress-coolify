...
function update_wp_config() {
  ...
  wp config set WP_SITEURL "https://$WORDPRESS_URL" --add --type=constant --path=/var/www/html
  wp config set WP_HOME "https://$WORDPRESS_URL" --add --type=constant --path=/var/www/html
